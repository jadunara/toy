...
String.prototype.paddingb = function(n, c)
{
	var val = this.valueOf();
	var bLength = this.bytelength();
	var mn = Math.abs(n);
	if ( mn == bLength ) {
		return val;
	} else if ( mn < bLength  ) {
	...
	var m = Math.max((Math.abs(n) - bLength) || 0, 0);
	var pad = Array(m + 1).join(String(c || ' ').charAt(0));
	return (n < 0) ? pad + val : val + pad;
};
...
String.prototype.cut = function (len){
	...
	var size = 0;
	var rIndex = s.bytelength();

	for ( var i = 0; i < s.length; i++) {
		var x = String.charByteSize(s.charAt(i));
		if ( x > 1) {
			x = 2;
		}
		size += x ;
...
// https://zzznara2.tistory.com/458
String.charByteSize = function (ch) {
...
String.prototype.sprintf.sb = function(ins, x)
{
	return String(ins).paddingb(x[2] + x[4], x[3]);
};
